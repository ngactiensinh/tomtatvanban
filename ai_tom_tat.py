...
# ==========================================
# CẤU HÌNH API KEY VÀ TÌM MODEL XỊN NHẤT
# ==========================================
...
with st.sidebar:
    ...
    try:
        ...
        # Quét ngầm danh sách Model (Không in ra màn hình cho đỡ rác)
        danh_sach_model = [m.name for m in genai.list_models() if 'generateContent' in m.supported_generation_methods]
        
        # Thuật toán: Ưu tiên chọn 2.5 -> 2.0 -> 1.5
        for phien_ban in ['2.5', '2.0', '1.5']:
            for name in danh_sach_model:
                if phien_ban in name and 'flash' in name and 'lite' not in name:
        ...
        # Kiểm tra xem Model có hỗ trợ Mắt thần không (Các bản 1.5, 2.0, 2.5 đều có)
        has_vision = any(v in model_name_chuan for v in ['1.5', '2.0', '2.5'])

        st.success("✅ Đã kết nối Chìa khóa an toàn.")
        st.info(f"🚀 AI đang dùng: **{model_name_chuan.replace('models/', '').upper()}**")
        
        if has_vision:
            st.markdown("<p style='color:#10b981; font-weight:bold;'>👁️ Trạng thái Mắt thần: ĐÃ BẬT</p>", unsafe_allow_html=True)
        
    except Exception as e:
        api_key = None
        st.error(f"⚠️ Lỗi kết nối Két sắt: {e}")
...
if nut_xu_ly:
    if not api_key:
        st.error("⚠️ Hệ thống đang thiếu chìa khóa VIP.")
         ...
         st.error("❌ Chìa khóa không có quyền dùng bất kỳ AI nào!")
    elif file_tai_len is None and not van_ban_goc.strip():
        st.warning("⚠️ Đồng chí chưa cung cấp file hay văn bản nào!")
    else:
        with st.spinner("🤖 Trợ lý AI đang thu thập và phân tích dữ liệu..."):
            hop_le = True
            ...
            if file_tai_len is not None:
                ...
                # NẾU LÀ PDF: BẮT BUỘC PHẢI DÙNG MẮT THẦN
                if file_ext == 'pdf':
                    if has_vision:
                        st.info("🔎 Đang dùng Mắt thần AI phiên bản mới nhất soi bản Scan PDF...")
                        try:
                        ...
                        st.error("❌ AI hiện tại không có Mắt thần để đọc file PDF Scan. Vui lòng dùng file Word.")
                        hop_le = False
            ...
            if hop_le:
                try:
                    ...
                    if "Tóm tắt" in che_do:
                        loi_dan = "Bạn là chuyên viên tổng hợp của cơ quan Đảng. Hãy đọc tài liệu và tóm tắt cốt lõi trong 3-5 câu súc tích. Văn phong trang trọng."
                    ...
                    # Dọn dẹp rác trên server Google
